...
SUTS = [
    ...
    Sut("spring-actuator-demo", JDK_8),
    Sut("spring-batch-rest", JDK_8),
...
for sut in SUTS:
        if isJava(sut):
            # copy jar files
            shutil.copy(os.path.join(DIST_DIR, sut.name + EM_POSTFIX), BASE_DIR)
            shutil.copy(os.path.join(DIST_DIR, sut.name + SUT_POSTFIX), BASE_DIR)
            ...
            shutil.copytree(os.path.join(DIST_DIR, sut.name), os.path.join(BASE_DIR, sut.name))
        else:
            raise Exception("Unexpected platform" + sut.platform)

shutil.copy(os.path.join(DIST_DIR, AGENT), BASE_DIR)
